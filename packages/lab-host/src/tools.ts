...
export function apply(ctx: Context): void {
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Show GPU resources: per-GPU free VRAM and running runs, plus the queue of runs waiting for cards (in queue order). Cards reserved by submitted-but-not-yet-started runs count as running (a card is held from submission until its run finishes).',
      parameters: {},
      output: { schema: { type: 'json' }, render: jsonRender },
      async execute(_args, exec) {
